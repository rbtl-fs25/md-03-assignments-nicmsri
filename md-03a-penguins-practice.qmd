...
```{r}
glimpse(penguins)
# penguins is divided in 8 columns (classes/variables) and 344 rows (observations/entries)
str(penguins)
# data has 3 types: factor, num (double), int

levels <- penguins %>% 
  sapply(levels) # levels(penguins$species)
```
...
```{r}
penguins_isnt_na_flipper <- penguins %>% 
  filter(!is.na(flipper_length_mm))

penguins_isnt_na_flipper %>% 
  group_by(species) %>% 
  summarise(flip_min=min(penguins_isnt_na_flipper$flipper_length_mm), flip_max=max(penguins_isnt_na_flipper$flipper_length_mm))
...
penguins %>% 
  group_by(species) %>% 
  filter(!is.na(flipper_length_mm)) %>% 
  summarise(flip_min=min(.$flipper_length_mm), flip_max=max(.$flipper_length_mm))
```
