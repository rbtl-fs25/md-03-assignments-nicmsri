...
```{r}
ggplot(sanitation_afg, aes(residence, percent, fill=varname_long))+
  geom_col()+
  facet_wrap(~year)+
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend? rural always has less basic sanitation
2.  What would you want to change? reorder of national and rural
3.  Why did we remove "safely managed sanitation services" from the data set in Task 3? uninteresting, no impact needed where there is safely managed sanitation services

## Task 7

1.  Run the code in the code chunk below.
2.  What do you observe when you look at the code and plot? that the safely managed sanitation services variables is uncorrelated to the others
...
ggplot(data = sanitation_2020,
  ...
  geom_histogram() +
  facet_grid(varname_long ~ residence, scales = "free_y") +
  scale_fill_colorblind() +
